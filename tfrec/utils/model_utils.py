...
def preprocess_and_split(
    df, shuffle=True, split=True, test_size=0.2, random_state=None
):
    ...
    **Encodes** & **Decodes** User and Item, each from `0 to len(user) and len(item)`.
    ...
    user_to_encoded = {
        user_id: encoded_id for encoded_id, user_id in enumerate(user_ids)
    }
    encoded_to_user = {
        encoded_id: user_id for encoded_id, user_id in enumerate(user_ids)
    }

    # Create encoding and decoding maps for items
    item_to_encoded = {
        item_id: encoded_id for encoded_id, item_id in enumerate(item_ids)
    }
    encoded_to_item = {
        encoded_id: item_id for encoded_id, item_id in enumerate(item_ids)
    }
